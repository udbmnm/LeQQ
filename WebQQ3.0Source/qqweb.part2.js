...
Jx().$package(function(f) {
    ...
    c = new f.Class({
        ...
        createDom: function() {
            ...
            this._toolbarHeight = 0; ! f.isUndefined(this.option.x) && !f.isUndefined(this.option.y) && this.setXY(this.option.x, this.option.y);
            ...
            for (var b in a) {
                ...
                default:
                    ...
                    case "widget":
                        switch (b) {
                        case "hasPinUpButton":
                            c.push(this._pinUpButton),
                            c.push(this._pinDownButton)
                        }
                    }
                }
                c.length && (f.array.forEach(c,
                function(a) {
        ...
        adjustSize: function(a, b, c, e) { ! f.isUndefined(c) && !f.isUndefined(e) && this.setXY(c, e);
            this.setWinWidth(a);
            this.setWinHeight(b);
            d.notifyObservers(this, "resize", this.getBodySize())
        },
        max: function() {
            if (! (this.getWindowFlags() & c.CONST.WINDOW_FLAG_FULLSCREEN)) this._restoreX = this._x,
        ...
        fullscreen: function() {
            if (this.getWindowFlags() & c.CONST.WINDOW_FLAG_NORMAL) this._restoreX = this._x,
            this._restoreY = this._y;
            this.setDisableDrag();
        ...
        setXY: function(a, b) { (a || a === 0) && this.setX(a); (b || b === 0) && this.setY(b)
        },
        setX: function(a) {
            this._x = a;
        ...
        setEnableDrag: function() {
            if (this.option.dragable) {
                if (this._dragController) {
                    ...
                    isOverRight: !0,
                    isLockCursorInScreen: !0
                }), d.addObserver(this._dragController, "move", this.observer.onMove)),
                d.addObserver(this._dragController, "beforeStart", this.observer.onBeforeDragStart),
                d.addObserver(this._dragController, "start", this.observer.onDragStart),
                d.addObserver(this._dragController, "end", this.observer.onDragEnd);
        ...
        onDrag: function(a) { (!f.browser.ie || !(f.browser.ie <= 6 && (a.target.tagName == "A" || a.target.parentNode && a.target.parentNode.tagName == "A"))) && this.startDrag([a.pageX, a.pageY])
        },
        startDrag: function(a) {
            ...
            b.mask.show();
            e.show(c);
            if (typeof a != "undefined") c = e.getClientXY(this.body),
            this._dragController.dragStartIn(a[0] + c[0], a[1] + c[1]);
            else {
                var h = function(a) {
        ...
        setLimite: function(a) {
            a = a || {};
            if (this.isLimited) this._leftMargin = a.leftMargin,
            this._topMargin = a.topMargin,
            this._rightMargin = a.rightMargin,
            this._bottomMargin = a.bottomMargin
        },
        ...
        hideWinBorder: function() {
            if (!this.isBorderHide) this.isBorderHide = !0,
            e.hide(this._window_bg),
            e.hide(this._titleBar),
            this.setWidth(this.getBodyWidth()),
            this.setHeight(this.getBodyHeight())
        },
        showWinBorder: function() {
            if (this.isBorderHide) this.isBorderHide = !1,
            e.show(this._window_bg),
        ...
        setSubWinNone: function() {
            if (this.subMode != 0) this.subMode = 0,
        ...
        removeSubordinate: function() {
            if (this.subWin) this.setSubMode(0),
            this.subWin = null,
            this.subWinWidth = 0,
            this.isSubWinFloat = !1
        },
        close: function() {
    ...
    b = new f.Class({
        init: function(a) {
            ...
            this._private = {
                ...
                clearTimeoutSaveRange: function() {
                    if (this._keyupTimer) window.clearTimeout(this._keyupTimer),
                    this._keyupTimer = 0
                },
                ...
                clearTimeoutClearNodes: function() {
                    if (this._clearNodesTimer) window.clearTimeout(this._clearNodesTimer),
        ...
        restoreRange: function() {
            if (this._lastRange) {
                ...
                if (a.addRange) a.removeAllRanges(),
                a.addRange(this._lastRange);
                else if (a = b.getRange()) a.setEndPoint("EndToStart", this._lastRange),
                a.collapse(!1),
                a.setEndPoint("EndToEnd", this._lastRange),
                a.select()
            }
        },
        insertHtml: function(a) {
            if (a !== "") {
                var c = this.getRange();
                if (c) if (c.pasteHTML) c.pasteHTML(a),
                c.collapse(!1),
                c.select();
                else {
                ...
                } else this.isEmpty() ? this._divArea.innerHTML = a: this._divArea.innerHTML += a,
                c = b.getRange(),
                a = this._divArea.lastChild,
                c.selectNode ? (c.setEndAfter(a), c.setStartAfter(a), f = b.getSelection(), f.removeAllRanges(), f.addRange(c)) : c.moveToElementText && (c.moveToElementText(a), c.collapse(!1), c.select());
        ...
        clearNodes: function() {
            for (var a = this._divArea,
            c, d, f = a.childNodes,
            e = f.length - 1,
            g; e >= 0; e--) if (g = f[e], g.nodeType !== 3) if (g.nodeType === 1) {
                if (g.nodeName !== "BR" && (!this.option.nodeFilter || !this.option.nodeFilter(g))) c = g.textContent || g.innerText || "",
                c !== "" ? (c = document.createTextNode(c), d || (d = c), a.replaceChild(c, g)) : a.removeChild(g)
...
Jx().$package("alloy.businessClass",
function(f) {
    var e = f.dom,
    d = f.event,
    g = new f.Class({
    ...
    },
    {
        ...
        initButtons: function() {
            ...
            this._buttons.cancelButton = this._cancelButton;
            ...
            this._buttons.okButton = this._okButton;
            ...
            this._buttons.nextButton = this._nextButton;
            ...
            this._buttons.previousButton = this._previousButton
        },
        createEvent: function() {
...
function(f) {
    ...
    },
    {
        ...
        initButtons: function() {
            ...
            this.option.hasPinUpButton && (this._pinUpButton.show(), this._pinDownButton.hide()),
            this.option.hasPinDownButton && (this._pinUpButton.hide(), this._pinDownButton.show())
        },
        createEvent: function() {
            this.setEnableDrag();
    ...
    this.Icon = new f.Class({
        ...
        showNotify: function(a) {
            if (this.option.isShowNotify && (a = Number(a), a = a > 999 ? 999 : a)) {
                ...
                this._el.appendChild(this._notifyEl),
                this._notifyEl.innerHTML = '<span class="appButton_notify_inner"></span>';
...
function(f) {
    ...
    },
    {
        text: "\u5378\u8f7d\u5e94\u7528",
        onClick: function(a, b, c) {
            if (a = c.getArgument().app) alloy.config.removeSetupAppList(a),
            alloy.util.report2app("appbutton|menudel")
        }
    }],
    a = function(b, c, f) {
        f ? j(b, c) : (d.on(b, "load",
        function() {
            if (!this._isLoad) this._isLoad = 1,
            a(b, c, !0);
            d.off(this, "load")
        }), b.src = c)
    },
        ...
        init: function(b, n) {
            ...
            if ((n.id == "appMarket" || n.id == "diskExplorer" || n.id == "messageCenter") && f.browser.ie == 6) j(this._iconImgEl, this.option.icon.url);
                ...
                if (q) {
                    ...
                    var s = "",
                    t = f.browser.ie == 6,
                    s = q.type || q; (s & 100) > 0 ? (s = r > 99999 ? p + q.bigIcon: v + "big.png", t ? a(o, s) : o.src = s) : (s & 10) > 0 ? (s = r > 99999 ? p + q.midIcon: v + "mid.png", t ? a(o, s) : o.src = s) : (s & 1) > 0 ? (s = r > 99999 ? p + q.smallIcon: v + "small.png", e.addClass(l, "appButton_appIcon_warp"), e.addClass(o, "appButton_smallIcon"), t ? a(o, s) : o.src = s) : t ? a(o, alloy.CONST.CDN_URL + "style/images/big.png") : o.src = alloy.CONST.CDN_URL + "style/images/big.png"
                }
            }
...
function(f) {
    ...
    },
    {
        ...
        onClick: function(a, b, c) {
            ...
            if (alloy.portal.getLoginLevel() != alloy.CONST.LOGIN_LEVEL_ALL || m({
                t: "uin",
                id: a
            })) alloy.portal.getLoginLevel() == alloy.CONST.LOGIN_LEVEL_ALL ? qqweb.portal.runApp("chatLogViewer", a) : alloy.portal.getPtwebqq() ? alloy.layout.alert("\u60a8\u9700\u8981\u767b\u5f55QQ\u540e\uff0c\u624d\u80fd\u53d1\u8d77\u4f1a\u8bdd\u6216\u67e5\u770b\u6d88\u606f\u3002",
            function() {
                qqweb.portal.runApp(50)
            }) : alloy.layout.alert("\u60a8\u9700\u8981\u767b\u5f55QQ\u540e\uff0c\u624d\u80fd\u53d1\u8d77\u4f1a\u8bdd\u6216\u67e5\u770b\u6d88\u606f\u3002",
            function() {
    ...
    },
    {
        ...
        onClick: function(a, b, d) {
            ...
            }) : qqweb.rpcService.sendGetFriendUin2(a, 4,
            function(a) {
    ...
    },
    {
        ...
        onClick: function(a, b, c) {
            ...
            if (alloy.portal.getLoginLevel() != alloy.CONST.LOGIN_LEVEL_ALL || n({
                ...
                gcode: a,
                from: "group"
            }) : alloy.portal.getPtwebqq() ? alloy.layout.alert("\u60a8\u9700\u8981\u767b\u5f55QQ\u540e\uff0c\u624d\u80fd\u53d1\u8d77\u4f1a\u8bdd\u6216\u67e5\u770b\u6d88\u606f\u3002",
            function() {
                qqweb.portal.runApp(50)
            }) : alloy.layout.alert("\u60a8\u9700\u8981\u767b\u5f55QQ\u540e\uff0c\u624d\u80fd\u53d1\u8d77\u4f1a\u8bdd\u6216\u67e5\u770b\u6d88\u606f\u3002",
            function() {
    ...
    m = function(a) {
        if (EQQ.Model.BuddyList.isBuddy(a.id)) return ! 0;
            ...
            var b = a.id;
            alloy.layout.confirm("\u60a8\u4e0eTa\u5df2\u89e3\u9664\u597d\u53cb\u5173\u7cfb\uff0c\u662f\u5426\u5220\u9664\u684c\u9762\u56fe\u6807\uff1f",
            function() {
                alloy.fileSystem.deleteFile(a, null, null, null, !0)
            },
            {
                onCancel: function() {
                    alloy.layout.confirm("\u662f\u5426\u91cd\u65b0\u6dfb\u52a0\u4e3a\u597d\u53cb\uff1f",
                    function() {
                        qqweb.rpcService.sendGetFriendUin2(b, 5,
                        function(a) {
    ...
    n = function(a) {
        if (EQQ.Model.BuddyList.getGroupByCode(a.id)) return ! 0;
            ...
            var b = a.id;
            alloy.layout.confirm("\u60a8\u5df2\u4e0d\u5c5e\u4e8e\u8be5\u7fa4\uff0c\u662f\u5426\u5220\u9664\u684c\u9762\u56fe\u6807\uff1f",
            function() {
                alloy.fileSystem.deleteFile(a, null, null, null, !0)
            },
            {
                onCancel: function() {
                    alloy.layout.confirm("\u662f\u5426\u91cd\u7533\u8bf7\u52a0\u5165\u8be5\u7fa4\uff1f",
                    function() {
                        qqweb.portal.runApp("buddyFinder", {
                            from: "group",
                            directJoin: b
    ...
    l = function(a) {
        ...
        alloy.portal.getLoginLevel() == alloy.CONST.LOGIN_LEVEL_ALL ? c.t == b.BUDDY ? m(c) && (EQQ.handleNotification(c.id, "single") || WebqCore.api.call(["chat", ["single", c.id]])) : n(c) && (EQQ.handleNotification(c.gid, "group") || WebqCore.api.call(["chat", ["group", c.id]])) : alloy.portal.getPtwebqq() ? alloy.layout.alert("\u60a8\u9700\u8981\u767b\u5f55QQ\u540e\uff0c\u624d\u80fd\u53d1\u8d77\u4f1a\u8bdd\u6216\u67e5\u770b\u6d88\u606f\u3002",
        function() {
            var a = "single";
            c.t == b.GROUP && (a = "group");
...
function(f) {
    ...
    m = function(a) {
        var c = a.getArgument();
        if (c.file.t == b.FILE) {
            if (c.isFragment() || c.isUploading()) a.getItemAt(0).disable(),
            a.getItemAt(1).disable(),
            a.getItemAt(3).disable(),
            a.getItemAt(4).disable(),
            a.getItemAt(5).disable();
    ...
    l = {
        ...
        onGetFileDownLoadSuccess: function(a) {
            a.retcode == 0 && a.result && a.result.code == 0 ? alloy.storage.receiveFile(a.result.objs[0]) : alloy.layout.confirm("\u6587\u4ef6\u635f\u574f\u6216\u5df2\u88ab\u5220\u9664\uff0c\u662f\u5426\u5c06\u5176\u5728Q+ Web\u4e2d\u5220\u9664\uff1f",
            function() {
        ...
        onGetFileViewSuccess: function(a) {
            ...
            } else alloy.layout.confirm("\u6587\u4ef6\u635f\u574f\u6216\u5df2\u88ab\u5220\u9664\uff0c\u662f\u5426\u5c06\u5176\u5728Q+ Web\u4e2d\u5220\u9664\uff1f",
            function() {
                alloy.desktopFile.deleteFile(a.arguments.fileList[0])
            })
        },
    ...
    },
    {
        init: function(a, b) {
            ...
            d.addObserver(alloy.flashUploadManager, "FileUploadComplete", e.onFileUploadComplete),
            d.addObserver(e, "destroy",
            function() {
        ...
        getFileIconClass: function(b) {
            if (!this._isUploading && this.file.size != this.file.cur_size) return this._isFragment = !0,
            "fragment";
        ...
        showUploadBar: function() {
            ...
            this._uploadBar.appendChild(this._uploadProcess),
            this._el.appendChild(this._uploadBar);
...
function(f) {
    ...
    j = {
        ...
        onFileProcessed: function(a) {
            if (a = e.getIcons(a.id, a.t)) for (var b = 0,
            c = a.length; b < c; b++) a[b].enable()
        },
        onUpdateNotifyNumber: function(a) {
            var b = e.getIcons(a.id, a.t);
            if (b) for (var a = a.notifyNumber || 0,
            c = 0,
            d = b.length; c < d; c++)(b[c].notifyNumber = a) ? b[c].showNotify(a) : b[c].hideNotify()
        },
...
function(f) {
    ...
    n, l, o, r, q = {
        ...
        onIndicatorClick: function(c) {
            ...
            case "switch":
                ...
                default:
                    c = Number(d),
                    alloy.desktopManager.setCurrentDesktop(c),
                    alloy.util.report2qqweb("navbar|" + c)
                }
            }
        },
        ...
        onSelfInfoReady: function(a) {
            if (a && a.uin) k.title = "\u7f16\u8f91\u4e2a\u4eba\u8d44\u6599",
            h.alt = "\u7f16\u8f91\u4e2a\u4eba\u8d44\u6599",
            h.src = alloy.util.getUserAvatar(a.uin, 1) + "&t=" + (new Date).getTime()
        },
    ...
    this.init = function() {
        ...
        if (f.browser.mobileSafari || !alloy.portal.isWebTop()) b.addClass(a, "no_sysbtn"),
        b.setStyle(a, "width", "240px");
        else {
            var e = j,
            l = b.node("div", {
    ...
    this.showRunWebTopTip = function() {
        ...
        }), l.setTitle("\u63d0\u793a"), l.setContent('\u4f7f\u7528\u65b0\u7248"\u589e\u5f3a\u7248"\u83b7\u5f97\u66f4\u4f73<br/>\u4f53\u9a8c'), l.showButton("ok", "\u7acb\u5373\u4e0b\u8f7d"), c.addObserver(l, "onBubbleOkBtnClick", e)),
        l.show({
    ...
    this.setNavBarOnTop = function(c, d, f) { (m = c) ? b.setStyle(a, "zIndex", alloy.layout.getTopZIndex(4)) : b.setStyle(a, "zIndex", "12");
        if (d) {
            if (alloy.portal.getLoginLevel() > alloy.CONST.LOGIN_LEVEL_NONE) alloy.config.configList.navTop = c ? 1 : 0,
            f ? alloy.rpcService.sendMSetConfigDelay({
                data: {
                    0 : {
...
function(f) {
    ...
    this.init = function() {
        ...
        q.init();
        g.addObserver(qqweb.layout, "clickDesktop",
        function() {
            e.hide()
        });
        a = !0
    ...
    var q = {
        ...
        fillResult: function(a) {
            ...
            for (l in this.listDomArr) a = this.listDomArr[l],
            g.off(a, "click"),
            g.on(a, "click",
            function(a) {
                a.preventDefault();
    ...
    v = function() {
        ...
        l = function(d, l) {
            if (l.method == "GET" && l.cacheTime) {
                ...
                q = c(d, l),
                j = e(q, g);
                if (j) f.info("url: " + d + " has a cache.", "AppMarketRequest"),
                l.onSuccess.call(l.context, j);
                else {
                    var h = l.onSuccess;
            ...
            } else f.info("url: " + d + " Width no cache!", "AppMarketRequest"),
            a(d, l)
        };
...
function(f) {
    ...
    },
    {
        init: function(a) {
            ...
            b.onAccept && d.addObserver(this.Window, "clickOkButton",
            function() {
                b.onAccept.apply(this)
            });
            b.onClose && d.addObserver(this.Window, "close",
            function() {
    ...
    },
    {
        init: function(a) {
            ...
            if (b.onAccept) {
                var e = this;
                d.addObserver(this.Window, "clickOkButton",
                function() {
                    b.onAccept.apply(e);
                    c = !0
                })
            }
            b.onCancel && d.addObserver(this.Window, "close",
            function() {
...
Jet().$package("alloy.businessClass",
function(f) {
    var e = f.dom,
    d = f.string,
    g = f.event,
    ...
    this.ShareFriends = new f.Class({
        init: function(a) {
            ...
            r = this.observer = {
                friendClickHanler: function(a) {
                    ...
                    if (b.style.display != "block") {
                        if (o.share_list.length < 10) o.addItem(c, "friend", a.children[1].innerHTML),
                        b.style.display = "block"
                    } else b.style.display = "none",
                    o.deleteItem(c, "friend");
                    o.renderShareList();
                ...
                onSendButtonClick: function(a) {
                    a.preventDefault();
                    if (o.share_list.length > 0) g.notifyObservers(o, "clickSendButton", d.cutByBytes(o._share_input.value, 100)),
                    r.onClearSelection(),
                    o._share_send.className = "sharef_submit sharef_disabled"
                },
            ...
            a.enableDrag && (new f.ui.Drag(m), g.on(e.id("sharef_popup_content_" + h), "mousedown",
            function(a) {
                a.stopPropagation()
            }));
            e.show(m);
        ...
        initMultilist: function(a, b) {
            ...
            for (e = 0; e < b.length; ++e) {
                ...
                var j = b[e].list;
                for (g = 0; g < j.length; ++g) f.push('<li item_id="'),
                f.push(j[g].id),
                f.push('"><span class="sharef_item_icon '),
                f.push(j[g].icon || b[e].icon),
                f.push('"></span><span class="sharef_nick">'),
                f.push(j[g].title),
                f.push('</span><span class="sharef_selected_icon"></span></li>');
                f.push("</ul></div>")
            }
    ...
    this.App = new f.Class({
        ...
        run: function(c) {
            ...
            if (window.webTop && this.option.id == 12) alloy.layout.alert("\u5f88\u62b1\u6b49\uff0c\u6b64\u5e94\u7528\u6682\u4e0d\u652f\u6301\u5ba2\u6237\u7aef\u3002", null, {
                ...
                need: j
            }) : (j = this.option.id, c.runFrom != "url" && alloy.portal.setReRunAppList([[c.id, qqweb.portal.getUrlOption(c.id) || {}]]), this.option.loginLevel > alloy.CONST.LOGIN_LEVEL_NOCHAT ? alloy.layout.showLoginWindow(j, !0) : alloy.layout.showLoginWindow(j, !1));
            else if (this.option.id == "56" && !f.sound.isReady) f.browser.plugins.flash < 9 ? alloy.layout.alert("\u5f88\u62b1\u6b49\uff0c\u53ef\u80fd\u7531\u4e8eFlash\u7248\u672c\u8fc7\u4f4e\uff0c\u97f3\u4e50\u76d2\u5b50\u65e0\u6cd5\u4f7f\u7528\uff0c<br />\u5efa\u8bae\u60a8\u5b89\u88c5\u6700\u65b0\u7248Flash\u540e\u4f7f\u7528\u3002") : g.addObserver(f.sound, "ready",
            function() {
                alloy.system.runApp("56")
            });
            else if (f.browser.ie && this.option.id == "13" && !this.detectActiveX()) alloy.layout.alert("\u6b64\u5e94\u7528\u9700\u8981\u63d2\u4ef6\u652f\u6301\uff0c\u70b9\u51fb\u786e\u5b9a\u83b7\u53d6\u5b89\u88c5",
            function() {
                ...
                if (this.isRunning()) {
                    ...
                    if (this.option.appType !== 1 && this.option.appType === 2) {
                        ...
                        if ((!this.option.ieOnly || f.browser.ie) && (!f.browser.ie || this.option.id != "13" || this.detectActiveX())) {
                            if ("exinfo" in j && (f.isObject(j.exinfo) ? j.exinfo: f.json.parse(j.exinfo)).powerLevel > 0) k = j.appKey = Math.random(),
                            h = j.wid = encodeURIComponent(f.json.stringify({
                            ...
                            g.addObserver(this.startingAnimationApp, "end",
                            function() {
                                d.hide(a._starting)
                            });
                            ...
                            this.isMouseDown = !1;
                            g.on(this._iframeDragResizeMask, "mousedown",
                            function() {
                                a.isMouseDown = !0
                            });
                            g.on(this._iframeDragResizeMask, "mouseup",
                            function() {
                                if (a.isMouseDown) d.hide(a._iframeDragResizeMask),
                                a.isMouseDown = !1
                            });
                            g.addObserver(this.window, "dragEnd",
                            function() {
                                if (a.isMouseDown) d.hide(a._iframeDragResizeMask),
                                a.isMouseDown = !1
                            });
                            ...
                            a.alterDom.innerHTML = "<div class='appIframeAlter'></div><div  class='appIframeAlterTxt'>\u8fd0\u884c\u4e2d\uff0c\u70b9\u51fb\u6062\u590d\u663e\u793a :)</div>",
                            this.window.body.appendChild(a.alterDom);
                            g.on(a._restart, "click",
                            function() {
                    ...
                    this.option.windowMode === "single" && g.addObserver(this, "runAgain",
                    function() {
                        a.window.setCurrent()
                    });
        ...
        createWindow: function(b) {
            ...
            var h = this._windowObserver = {
                ...
                onWindowClose: function() {
                    ...
                    if (a.exit() === !1) return g.addObserver(a, "exit", h.onAppExit),
                    !1;
                    else {
                        if (a._iframe) {
        ...
        exit: function() {
            ...
            else g.removeObserver(this, "runAgain"),
            g.notifyObservers(this, "exit"),
            this.destroy(),
            g.removeObserver(this, "exit")
        },
        destroy: function() {
...
function(f) {
    ...
    c = this.createShareFriends = function(a) {
        ...
        if (alloy.portal.getLoginLevel() < 3) alloy.layout.alert("\u60a8\u5c1a\u672a\u767b\u5f55QQ\uff0c\u8bf7\u767b\u5f55\u540e\u518d\u8fdb\u884c\u8be5\u64cd\u4f5c\u3002",
        function() {
            ...
            var o = {
                ...
                onShareByC2C: function() {
                    ...
                    if (c && c.state == "offline") h.show("\u5df2\u79bb\u7ebf", "error");
                        ...
                        for (var d = function(b) {
                            var c = a.text; ! f.isUndefined(a.url) && a.url != "" && (c += a.url);
                ...
                onInviteInstallApp: function(a) {
                    for (var c = e.share_list,
                    d = alloy.appconfig.getAllConfig(a._appId), a = {
                        templateId: 1,
                        list: [],
                ...
                onShareApp: function(a) {
                    for (var b = e.share_list,
                    c = [], d = 0, s = b.length; d < s; d++) c.push(b[d].uin);
                    b = {
                        share_type: a.type,
                        friend_uins: "[" + c.join() + "]",
                        dst_appid: a.gaid,
...
function(f) {
    ...
    m = new f.Class({
        init: function(a) {
            ...
            var b = this;
            g.addObserver(a.window, "setCurrent",
            function() {
        ...
        showPanel: function(a) {
            if (this._currentPanel) this._currentPanel.hide(),
            this._currentPanel = null;
            if (a = this.get(a)) this._alterMode && this._window.setNotCurrent(),
            a.show(),
            this._currentPanel = a
        },
        ...
        init: function(a) {
            ...
            function(a) {
                ...
                if (a = alloy.util.getActionTarget(a, 2, "cmd")) {
                    ...
                    if (alloy.portal.getLoginLevel() < 2 && b != "detail" && b != "home") alloy.layout.alert("\u60a8\u5c1a\u672a\u767b\u5f55QQ\uff0c\u8bf7\u767b\u5f55\u540e\u518d\u8fdb\u884c\u8be5\u64cd\u4f5c\u3002",
                    function() {
            ...
            function(a) {
                ...
                if (a.target.className == "window_toolBar") {
                    c.panelManager.hideCurrent();
                    if (c.currentToolbarBtn) d.removeClass(c.currentToolbarBtn, "app_toolbar_button_hover"),
                    c.currentToolbarBtn = null;
                    c.panelManager._alterMode && c.window.setCurrent()
                }
            });
            g.addObserver(b, "setCurrent",
            function() {
                if (c.currentToolbarBtn) d.removeClass(c.currentToolbarBtn, "app_toolbar_button_hover"),
                c.currentToolbarBtn = null
            });
            ...
            alloy.rpcService.getAppInfo(this._appId, ["avgVote", "myVote", "vote1", "vote2", "vote3", "vote4", "vote5"],
            function(a) {
                if (a.retcode == 0 && a.result && a.result.resultData) a = a.result.resultData,
                c.appVote = {
                    avgVote: a.avgVote,
                    myVote: a.myVote,
        ...
        toggleScorePanel: function() {
            ...
            if (!this._scorePanel) {
                ...
                m = this.setVote = function(a) {
                    ...
                    if (a.myVote > 0) k(a.myVote),
                    t.innerHTML = "\u60a8\u7684\u8bc4\u5206\u4e3a " + a.myVote + " \u5206"
                };
                g.on(e, "mouseover",
                function(a) {
                ...
                function(b) {
                    ...
                    b = c.getAttribute("star"),
                    b = Number(b),
                    a.appVote.myVote < 1 && a.appVote.voteCount++,
                    a.appVote.myVote = b,
                    m(a.appVote),
                    alloy.rpcService.setAppVote({
                        appid: a._appId,
                        value: b,
                        type: 1
                    }),
            ...
            if (!this._sharePanel) {
                ...
                p = {
                    ...
                    onQQfreindsItemClick: function(c) {
                        c.preventDefault();
                        a._sharePanel.toggleShow();
                        if (a.currentToolbarBtn) d.removeClass(a.currentToolbarBtn, "app_toolbar_button_hover"),
                        a.currentToolbarBtn = null;
                        ...
                        alloy.rpcService.cgiSend(alloy.CONST.JAVA_CGI_URL + "cgi/qqweb/pengyou/share.do", {
                            ...
                            onSuccess: function(b) {
                                b.retcode == 21 ? h("\u8bf7\u52ff\u91cd\u590d\u63d0\u4ea4", "error", a.window) : b.retcode == -10002 ? (b = d.node("a", {
                                    href: "###",
                                    "class": "share_box_hintLink"
                                }), b.innerHTML = "\u8bf7\u5148\u5f00\u901a\u670b\u53cb", g.on(b, "click",
                                function(a) {
                                    a.preventDefault();
                                    alloy.portal.openInWebBrowser({
    ...
    this.TitleToolBar = function(a) {
        ...
        p = {
            ...
            onToolBarClick: function(a) {
                a.preventDefault();
                alloy.portal.getLoginLevel() < 2 && alloy.layout.alert("\u60a8\u5c1a\u672a\u767b\u5f55QQ\uff0c\u8bf7\u767b\u5f55\u540e\u518d\u8fdb\u884c\u8be5\u64cd\u4f5c\u3002",
                function() {
        ...
        u = function() {
            alloy.layout.alert("\u60a8\u5c1a\u672a\u767b\u5f55QQ\uff0c\u8bf7\u767b\u5f55\u540e\u518d\u8fdb\u884c\u8be5\u64cd\u4f5c\u3002",
            function() {
                alloy.layout.showLoginWindow(k)
            }).setWindowCenteredRelative(window)
        },
            ...
            if (!F) {
                ...
                s = j.setVote = function(a) {
                    if (F) {
                        ...
                        if (a.myVote > 0) l(a.myVote),
                        p.innerHTML = "\u60a8\u7684\u8bc4\u5206\u4e3a " + a.myVote + " \u5206"
                    }
                };
                g.on(e, "mouseover",
                function(a) {
                ...
                function(a) {
                    ...
                    a = b.getAttribute("star"),
                    a = Number(a),
                    x.myVote < 1 && x.voteCount++,
                    x.myVote = a,
                    s(x),
                    alloy.rpcService.setAppVote({
                        appid: k,
            ...
            if (D) D.isShow() ? (t.className = "window_toolButtonBar", w.hidePanel("comment")) : w.showPanel("comment");
                ...
                var a = {
                    onAddComment: function() {
                        ...
                        alloy.rpcService.createAppComment(a,
                        function(a) {
            ...
            if (B) B.isShow() ? (t.className = "window_toolButtonBar", w.hidePanel("complain")) : w.showPanel("complain");
                ...
                var a = {
                    onAddComplain: function() {
                        ...
                        alloy.rpcService.createAppComplain(a,
                        function(a) {
        ...
        alloy.rpcService.getAppInfo(k, ["avgVote", "myVote", "vote1", "vote2", "vote3", "vote4", "vote5"],
        function(a) {
            if (a.retcode == 0 && a.result && a.result.resultData) a = a.result.resultData,
            x = {
                avgVote: a.avgVote,
    ...
    g = {
        onMessage: function(b) {
            if (b.mt == 1) {
                ...
                a = Number(b.m);
                if (c == 50) {
                    if (alloy.system.getLoginLevel() < 3) c = b.ext.id,
                    alloy.config.configList.isNoContactNotify || (b = {
                        id: c,
                        t: d.BUDDY,
                        notifyNumber: a
                    },
                    alloy.iconFactory.updateNotifyNumber(b))
                } else(b = alloy.portal.getAppConfig(c)) && (b.notifyNumber = a),
                b = {
                    id: c,
                    t: d.APP,
                    notifyNumber: a
                ...
                alloy.iconFactory.updateNotifyNumber(b),
                alloy.qplus.onWebMessage({
                    appId: c,
                    number: a
                })
            } else if (b.mt == 2) c = b.appid,
            alloy.qplus.onWebMessage({
                appId: c,
                msg: b.m,
                time: ""
...
Jx().$package("alloy.messageSystem",
function(f) {
    var e = f.event,
    d = this,
    g = [],
    ...
    this.handleNotificationBySource = function(b) {
        for (var a = !1,
        f = 0; f < g.length; f++) if (g[f].from == b) {
            var a = !0,
            k = g[f];
            g.splice(f, 1);
            f--;
...
function(f) {
    ...
    this.register = function(b, c, d, e) {
        ...
        if (!k[b]) {
            f = f.types;
            for (var h in f) g = f[h],
            !j[g] ? j[g] = [b] : j[g].push(b)
        }
        k[b] = {
            onMessage: c || null,
            onHandled: d || null,
    ...
    this.init = function() {
        ...
        this.notifierList = k,
        this.messageType2Ids = j,
        this.notifierConfigs = a,
        e.addObserver(alloy.messageSystem, "MessageReceive", h.onMessageReceive),
        e.addObserver(alloy.messageSystem, "MessageHandled", h.onMessageHandled),
        new m,
        new n,
        alloy.app.msgBubble.run(),
        alloy.app.messageManager.run(),
        new l,
        f.browser.ie == 9 && new r,
        f.browser.ie && /(tencenttraveler)|(qqbrowser)|(maxthon)|(se 2.x)/.test(navigator.userAgent.toLowerCase()) && alloy.portal.runApp("ienotification"),
        window.webTop && new o
    };
    ...
    l = new f.Class({
        init: function() {
            ...
            if (a.hasSupport()) a.requestPermission(),
            this._notifier = a,
            this._notifierManage = [],
            this._popupHideTime = 5E3,
            alloy.notifier.register("desktopNotification", this.observer.onMessageReceive, null, this)
        },
        ...
        observer: {
            onMessageReceive: function(a) {
                if (this.checkIsNeedNotify(a)) {
                    var b, c;
                    b = a.body;
                    for (var e = this._notifierManage,
                    f = this._notifier; e.length >= 4;) {
                        var g = e.shift();
...
function(f) {
    ...
    alloy.sound = {
        ...
        function(b, c) {
            ...
            var c = c || !1,
            a = g[b];
            if (!a && !c) a = g[b] = new d(b),
            a.load();
            else if (c) a = this._IpadAudioObj,
            a.play(!0, b);
            return a
        }: function(b, c) {
            if (!d.isReady || this.isMute() || !b) return ! 1;
...
function(f) {
    ...
    a, j;
    try {
        a = new ActiveXObject("CrossDomainCtrl.CrossDomain"),
        j = !0
    } catch(k) {}
    var h = {
        system: {
            alert: {
                powerLevel: 1,
                api: function(a) {
                    alloy.layout.alert(b.encodeHtml(a.msg),
                    function() {
                        a.seq && r(a.seq, !0)
                    },
                    a.option);
            ...
            confirm: {
                powerLevel: 1,
                api: function(a) {
                    alloy.layout.confirm(b.encodeHtml(a.msg),
                    function() {
                        a.seq && r(a.seq, !0)
                    },
                    {
            ...
            setWallpaper: {
                ...
                api: function(a) {
                    ...
                    if (/^http:\/\/([\w\-]+\.)+[\w\-]+(\/[\w\-\.\/?%&=]*)?$/.test(a.url)) {
                        var c = a.url,
                        d = b[a.type] || "fill";
                        alloy.layout.confirm("\u7ee7\u7eed\u64cd\u4f5c\u5c06\u4fee\u6539\u60a8\u7684\u684c\u9762\u4e3b\u9898\u8bbe\u7f6e\uff0c\u662f\u5426\u7ee7\u7eed\uff1f",
                        function() {
    ...
    q = function(a, b) {
        ...
        if (d = n(a, h)) {
            ...
            if (alloy.system.getAllConfig(b.appId).powerLevel >= d) {
                ...
                j = c[1];
                b.param = b.param || {};
                if (b.seq) b.param.seq = b.seq,
                o[b.seq] = b;
                if (g == "app") e = alloy.system.getApp(b.appId),
                c = function(b) {
                    var c;
    ...
    s = function(a, b) {
        ...
        if (d && h.powerLevel >= d) try {
            var j;
            if (j = (c[e] || 0)[b.seq]) g.removeObserver(j[0], j[1], j[2]),
            delete c[e][b.seq]
        } catch(k) {
    ...
    z = function(a) {
        ...
        c = parseInt(b.appid),
        d;
        if (c && (d = alloy.system.getCacheOpenkey(c))) b.gaid = d.gaid,
        b.app_openid = d.openId,
        b.app_openkey = d.openKey;
    ...
    this.postMessage = function(b, c) {
        ...
        if (!f.isUndefined(b) && !f.isUndefined(b.wid) && !f.isUndefined(b.host) && !f.isUndefined(c)) {
            ...
            if (h && k.test(e)) if (f.isUndefined(window.postMessage)) if (j) e = eval(e),
            a.PostMessageEx(h, e);
            else {
                ...
                if (e = eval(e)) g = g.indexOf("http://") != -1 ? g: "http://" + g,
                e.postMessage(h, g);
    ...
    } else g.addEventListener(window, "message",
    function(a) {
    ...
    C = {
        "system.qplusID": ["app.openId",
        function(a) {
            a.qplusID = a.openId;
            delete a.openId;
            return a
        }],
        "system.shareApp": ["app.shareApp",
        function(a) {
            return a
        }]
    },
    ...
    I = function(a, b, c) {
        ...
        if (d = c[a]) {
            ...
            if (!c) return d(a, b),
            !0;
            for (var e in b.param) if (b.param.hasOwnProperty(e) && c.hasOwnProperty(e)) return d(a, b),
            !0
        }
        return ! 1
    },
...
Jx().$package(alloy.app.msgBubble = new alloy.businessClass.App(alloy.portal.getSystemConfig("msgBubble")),
function(f) {
    var e = f.dom,
    d = f.event,
    g = this,
    ...
    t,
    w,
    z = '\t\t<div id="messageBubble_bubblePanel" class="bubblePanel" >\t\t\t<span id="messageBubble_bubblePanel_setting" class="icon setting" title="\u6d88\u606f\u7ba1\u7406\u5668"></span>\t\t\t<div id="messageBubble_bubblePanel_message" class="item"></div>\t\t</div>\t\t<div id="messageBubble_bubbleMsgList" class="bubbleMsgList" >\t\t\t<h3>\u672a\u8bfb\u6d88\u606f(<span id="messageBubble_bubbleMsgList_userCount" class="count">10</span>)</h3>\t\t\t<div class="bubbleMsgListInner">\t\t\t\t<div id="messageBubble_bubbleMsgListContainer" class="bubbleMsgListContainer">\t\t\t\t\t<ul id="messageBubble_bubbleMsgList_ul">\t\t\t\t\t</ul>\t\t\t\t</div>\t\t\t\t<div class="bubbleMsgButtons">\t\t\t\t\t<a href="#cancel" id="messageBubble_bubbleMsgList_cancelNotifyButton" class="cancelNotify">\u5ffd\u7565\u63d0\u9192</a>\t\t\t\t\t<a href="#viewall" id="messageBubble_bubbleMsgList_viewAll" class="viewAll">\u67e5\u770b\u5168\u90e8</a>\t\t\t\t</div>\t\t\t</div>' + (f.browser.ie ? '<iframe class="flash_mask_iframe" border="0"></iframe>': "") + "</div>";
    ...
    H = function() {
        ...
        g = e.getWidth(l),
        h = e.getHeight(l); ! f.isUndefined(a) && a.length == 2 ? (b = a[1], a = a[0], b > d - h - 30 && (b = d - h - 30), a > c - g && (a = c - g)) : (b = d - h - 35, a = c / 2 - g / 2);
    ...
    x = function(a) {
        var b = k[a];
        if (b) alloy.messageSystem.handleNotification(a),
        a = b.message.type,
        a == "SingleChat" ? alloy.util.report2im("msgbubble|viewc2cmsg") : a == "GroupChat" ? alloy.util.report2im("msgbubble|viewgroupmsg") : a == "DiscuChat" && alloy.util.report2im("msgbubble|viewdisupmsg")
    },
    C = function(a) {
    ...
    d.addObserver(this, "run",
    function() {})
});
Jx().$package(alloy.app.messageManager = new alloy.businessClass.App(alloy.portal.getSystemConfig("messageManager")),
function(f) {
    var e = f.dom,
    d = f.event,
    g = f.string,
    ...
    l = {
        ...
        onMsgLiClick: function(a) {
            ...
            case "BuddyAdd":
                ...
                break;
            case "GroupJoin":
                alloy.portal.runApp("groupSystemMsg", b.extra),
                alloy.util.report2im("qqpanel|contacts|msgmgr|sys")
            }
        },
        onWindowClose: function() {
            for (var b in n) d.off(n[b].msgAreaBtnTrigger, "click"),
            d.off(n[b].btn, "click");
            m = null;
            a = {}
        },
    ...
    v = function(b, c) {
        ...
        if (/(single)|(group)|(discu)/.test(b.type)) {
            var h = f.uin + f.type;
            if (a[h]) {
                var j = e.id("messagemmg_msg_" + a[h]);
                if (j) {
                    if (c) return;
        ...
        d.innerHTML = '            <div class="time">' + g.encodeHtml(f.time) + '</div>            <div class="from"><a href="###">' + b.avatar + '</a>                <span class="name"><a href="###">' + f.title + '</a></span>            </div>            <div class="msgbody">                <div class="msgcontent_container"><a class="msgcontent" href="###">' + f.content + "</a></div>            </div>";
        f = n[b.type];
        h = f.msgUl;
        h.firstChild ? h.insertBefore(d, h.firstChild) : h.appendChild(d);
        e.hide(f.noMsgTip);
        e.show(h);
        for (var k in n) f = n[k],
        d = f.msgUl.children.length,
        f.count.innerHTML = "" + d,
        f.count2.innerHTML = "" + d
    },
    p = function(a) {
    ...
    w = function(a) {
        ...
        if (!f.isUndefined(b)) for (var c = b.length - 1; c >= 0; --c) {
            ...
            WebqCore.api.call(["getGroupByCode", [d.g,
            function(a) {
        ...
        if (!f.isUndefined(a)) for (c = a.length - 1; c >= 0; --c) d = a[c],
        e = {
            from: "",
            to: "",
            type: "SingleChat",
        ...
        WebqCore.api.call(["getUserByUin", [d.tuin,
        function(a) {
            if (a) e.body.title = a.htmlShowName
        }]]),
        d = q(e),
    ...
    function() {
        if (j) j = !1,
        alloy.notifier.register("messageManager", l.onMessageReceive),
        d.addObserver(alloy.messageSystem, "MessageSourceRemove", l.onRemoveSource),
        d.addObserver(alloy.portal, "reset", l.onUinChange);
        else {
            if (!m) {
        ...
        if (!g) {
            ...
            case "userData":
                ...
                this.save = function(a, d) {
                    try {
                        c.load(b),
                        c.expires = (new Date( + new Date + 6048E5)).toGMTString(),
                        c.setAttribute(a, d),
                        c.save(b)
                    } catch(e) {}
                };
                ...
                this.clear = function() {
                    try {
                        c.load(b),
                        c.expires = (new Date( + new Date - 36E5)).toGMTString(),
                        c.save(b)
                    } catch(a) {}
                };
                this.flush = function() {
            ...
            case "flash":
                ...
                d = f.browser.ie ? e.id("sharedObject") || window.sharedObject: document.sharedObject;
                this.initStorage = function(a) {
                    d.init(a)
                };
...
function(f) {
    ...
    n = {
        ...
        sendQplusLogin: function() {
            ...
            alloy.rpcService.cgiSend(alloy.CONST.JAVA_CGI_URL + "cgi/top/qpluslogin", {
                ...
                data: a,
                onSuccess: function(a) {
                    if (a.retcode === 0) k = a.result.connProxy_ip,
                    h = a.result.connProxy_port,
                    d.setOnline()
                }
            })
        }
    ...
    l = {
        onQplusMessageHandled: function(b) {
            if (c) {
                ...
                if (b & m.SHARE) {
                    for (var b = Number.POSITIVE_INFINITY,
                    d = Number.NEGATIVE_INFINITY,
                    e = 0,
                    f; f = c[e]; e++) b = Math.min(b, f.time),
                    d = Math.max(d, f.time);
                    n.sendMarkShare({
                        msg_type: 2,
                        start_time: b,
        ...
        onAppRun: function(a) {
            if (!isNaN(a) && b) for (var c = 0,
            d; d = b[c]; c++) if (d.appId == a) {
                b[c].number = 0;
                b[c].count = 0;
    ...
    this.login = function() {
        if (!g) {
            ...
            try {
                b = f.json.parse(alloy.localStorage.read("appList")),
                f.isArray(b) || (b = [], a &= ~m.APP)
            } catch(d) {
                b = [],
                a &= ~m.APP
            }
            try {
                c = f.json.parse(alloy.localStorage.read("shareList")),
                f.isArray(c) || (c = [], a &= ~m.SHARE)
            } catch(e) {
                c = [],
                a &= ~m.SHARE
            }
            for (var h = 0,
            k = b.length; h < k; h++) b[h] && b[h].id && (j[b[h].id] = !0);
            h = 0;
