Jx().$package(alloy.app.explorer = new alloy.businessClass.App(alloy.portal.getSystemConfig("explorer")),
function(n) {
    var d = n.dom,
    f = n.event,
    ...
    this.folderList = k;
    var J = function(c) {
        c && n.browser.mobileSafari && n.platform.iPad.split(".")[0] >= 4 && c.addEventListener("touchstart",
        function(a) {
    ...
    D = function(c, a) {
        ...
        case j.APP:
            ...
            if (!d) return n.profile('createFileIcon. id="' + a.id + '" appConfig is null', "Explorer"),
    ...
    P = function(c) {
        ...
        f.on(d.id("quick_view_container_open_" + a), "click",
        function(b) {
            b.preventDefault();
            k[a] || (k[a] = new H(c));
        ...
        if (h == 0) o.innerHTML = "\u6587\u4ef6\u5939\u4e3a\u7a7a";
            ...
            for (var s, p = 0; p < h; ++p) {
                var r = b[p];
                if (s = D(o, r))(r.t == j.FILE || r.t == j.FOLDER) && s.contextMenuDisable(),
                E[r.id] = s
            }
        }
        ...
        var q = function() {
            ...
            c.x + 395 < N || c.x < 335 ? (e.quickViewPopup.setX(c.x + 85), e.box.setArrowPos(0, Math.max(c.y - a, 0))) : (e.quickViewPopup.setX(c.x - 340), e.box.setArrowPos(1, Math.max(c.y - a, 0)))
        };
    ...
    H = function(c) {
        ...
        function(a) {
            ...
            if ((a == g || a.parentNode.parentNode == g || a.parentNode == g) && alloy.desktopManager.getDesktopStatus() === alloy.desktopManager.DESK_STATUS.EDIT) alloy.desktopManager.setDesktopStatus(alloy.desktopManager.DESK_STATUS.NORMAL),
            alloy.util.report2qqweb("screen|ipad|edited")
        });
        f.on(g, "contextmenu",
        function(a) {
            a.stopPropagation();
        ...
        D = function(a) {
            ...
            case j.APP:
                ...
                if (!e) return n.profile('createFileIcon. id="' + item.id + '" appConfig is null', "Explorer"),
                alloy.fileSystem.deleteFile(a, null, null, null, !1),
                null;
                if (c = alloy.iconFactory.createIcon(j.APP, c, e)) A[a.id] = c,
                b.appColumnTitle.innerHTML = "\u5e94\u7528(" + ++b.appListCount + ")",
                d.show(b.appColumn);
                break;
            case j.BUDDY:
            case j.GROUP:
                ...
                c = B[a.id] = alloy.iconFactory.createIcon(a.t, c, a),
                b.contactColumnTitle.innerHTML = "\u8054\u7cfb\u4eba(" + ++b.contactListCount + ")",
                d.show(b.contactColumn)
            }
        ...
        this.deleteFileIcon = function(c) {
            ...
            case j.GROUP:
                var e = B[c.id];
                e && e.destroy();
                delete B[c.id]; --b.contactListCount;
    ...
    var l = {
        ...
        onFileAdd: function(c) {
            ...
            c = c.file,
            b = k[a];
            b && b.addFileIcon(c); (a = m[a]) && a.addFileIcon(c)
        },
        onFileDelete: function(c) {
            var a = c.parent.id,
            c = c.file,
            b = k[a];
            b && b.deleteFileIcon(c); (a = m[a]) && a.deleteFileIcon(c); (b = k[c.id]) && b._window.close()
        },
        ...
        onFileMove: function(c) {
            ...
            c = c.file,
            d = k[a];
            d && d.addFileIcon(c); (a = m[a]) && a.addFileIcon(c); (a = k[b]) && a.deleteFileIcon(c); (b = m[b]) && b.deleteFileIcon(c)
        },
        ...
        getFolderItemsError: function(c) { (c = k[c.arguments.obj.id]) && c.showLoading("\u4ece\u7f51\u76d8\u540c\u6b65\u6587\u4ef6\u5931\u8d25\uff01")
        },
        onExit: function() {},
